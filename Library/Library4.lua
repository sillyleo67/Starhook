...
function Library:MakeDraggable(Instance, Cutoff, IsMainWindow)
    ...
    if Library.IsMobile == false then
        ...
        Library:AddToRegistry(DragOutline, {
        ...
        Instance.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                local UsingOutline = Library.PerformanceDrag == true

                if UsingOutline then
                    DragOutline.Size = UDim2.fromOffset(Instance.AbsoluteSize.X, Instance.AbsoluteSize.Y)
                ...
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    if UsingOutline then
                        DragOutline.Position = NewPosition
                    else
...
do
    ...
    function BaseGroupboxFuncs:AddPreview(Idx)
        local Groupbox = self
        local Container = Groupbox.Container
        ...
        Library:Create("Frame", {
            ...
            Size = UDim2.new(0, 48, 0, 88);
            ZIndex = 7;
            Parent = PreviewInner;
        })

        local Help = Library:CreateLabel({
            Size = UDim2.new(1, -8, 0, 14);
            Position = UDim2.new(0, 4, 1, -18);
            TextSize = 12;
            Text = "Drag a label to reposition it";
            TextXAlignment = Enum.TextXAlignment.Left;
            ZIndex = 7;
            Parent = PreviewInner;
        })
        ...
        local Elements = {
            { Key = "Name", Text = "Name", DefaultX = 0, DefaultY = -66 };
            { Key = "HealthNumber", Text = "100", DefaultX = 34, DefaultY = -10 };
            { Key = "Distance", Text = "42m", DefaultX = 0, DefaultY = 38 };
            { Key = "Weapon", Text = "Weapon", DefaultX = 0, DefaultY = 52 };
            { Key = "Stance", Text = "Standing", DefaultX = 0, DefaultY = 66 };
        }

        for _, ElementInfo in Elements do
            local OptIdx = Idx .. ElementInfo.Key .. "Offset"
            local Default = { X = ElementInfo.DefaultX, Y = ElementInfo.DefaultY }

            local Label = Library:CreateLabel({
                Size = UDim2.new(0, 70, 0, 14);
                AnchorPoint = Vector2.new(0.5, 0.5);
                Position = UDim2.new(0.5, Default.X, 0.5, Default.Y - 10);
                TextSize = 12;
            ...
            Options[OptIdx] = {
                ...
                SetValue = function(self, Value)
                    if typeof(Value) == "table" and Value.X and Value.Y then
                        ...
                        Label.Position = UDim2.new(0.5, Value.X, 0.5, Value.Y - 10)
                    end
                end;
            }
            ...
            Label.InputBegan:Connect(function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    local StartOffset = Options[OptIdx].Value

                    while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1 or Enum.UserInputType.Touch) do
                        local Delta = Vector2.new(Mouse.X, Mouse.Y) - StartMouse
                        local NewX = math.clamp(StartOffset.X + Delta.X, -120, 120)
                        local NewY = math.clamp(StartOffset.Y + Delta.Y, -80, 80)

                        Options[OptIdx].Value = { X = NewX, Y = NewY }
                        Label.Position = UDim2.new(0.5, NewX, 0.5, NewY - 10)
...
do
    ...
    Library:AddToRegistry(WatermarkInline, {
        BackgroundColor3 = "MainColor";
    })
    ...
    function Library:SetWatermark(Text)
        ...
        Library.Watermark.Size = UDim2.new(0, X + 15, 0, (Y * 1.5) + 5)
...
do
    Library.Friends = {}
    Library.Priorities = {}
    Library.PerformanceDrag = false

    function Library:SetPerformanceDrag(Bool)
        Library.PerformanceDrag = Bool
    end

    function Library:ResetAllPanelPositions()
        if Library.ResetPlayerListPosition then
            Library.ResetPlayerListPosition()
        end

        if Library.ResetServerListPosition then
            Library.ResetServerListPosition()
        end
    end

    local function CreatePanelButton(Parent, Text, Position, Size)
    ...
    local function CreatePanel(Title, PanelWidth, PanelHeight)
        local Outer = Library:Create("Frame", {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderSizePixel = 0;
    ...
    local function AttachToWindow(Outer, PanelWidth, Side)
        local Enabled = true
        local ManuallyPositioned = false

        local function Reposition()
            if not (LibraryMainOuterFrame and LibraryMainOuterFrame.Parent) then
                return
            end

            Outer.Visible = Enabled and LibraryMainOuterFrame.Visible

            if ManuallyPositioned then
                return
            end

            local WindowPosition = LibraryMainOuterFrame.AbsolutePosition
            local WindowSize = LibraryMainOuterFrame.AbsoluteSize

            if Side == "Right" then
                Outer.Position = UDim2.fromOffset(WindowPosition.X + WindowSize.X + 8, WindowPosition.Y)
            else
                Outer.Position = UDim2.fromOffset(WindowPosition.X - PanelWidth - 8, WindowPosition.Y)
            end
        end

        Reposition()
        RunService.RenderStepped:Connect(Reposition)
        ...
        local function MarkManuallyPositioned()
        ...
        return SetEnabled, MarkManuallyPositioned, ResetPosition
    end

    local PanelMinVisible = 40

    local function ClampPanelPosition(Position, PanelSize)
        local ScreenSize = ScreenGui.AbsoluteSize
        local MinX = PanelMinVisible - PanelSize.X
        local MaxX = ScreenSize.X - PanelMinVisible
        local MinY = 0
        local MaxY = ScreenSize.Y - PanelMinVisible

        return UDim2.new(
            Position.X.Scale, math.clamp(Position.X.Offset, MinX, MaxX),
            Position.Y.Scale, math.clamp(Position.Y.Offset, MinY, MaxY)
        )
    end

    local function MakePanelDraggable(Outer, Cutoff, MarkManuallyPositioned)
        local DragOutline = Library:Create("Frame", {
            BackgroundTransparency = 1;
            BorderColor3 = Library.AccentColor;
            BorderMode = Enum.BorderMode.Inset;
        ...
        local Dragging, StartMouse, StartPosition, PanelSize
        ...
        Outer.InputBegan:Connect(function(Input)
            ...
            if (Mouse.Y - Outer.AbsolutePosition.Y) > (Cutoff or 25) then
                return
            end

            Dragging = true
            StartMouse = Vector2.new(Mouse.X, Mouse.Y)
            StartPosition = Outer.Position
            PanelSize = Outer.AbsoluteSize

            if Library.PerformanceDrag then
                DragOutline.Size = UDim2.fromOffset(PanelSize.X, PanelSize.Y)
                DragOutline.Position = StartPosition
                DragOutline.Visible = true
            end
        end)
        ...
        InputService.InputChanged:Connect(function(Input)
            ...
            local Delta = Vector2.new(Mouse.X, Mouse.Y) - StartMouse
            local NewPosition = ClampPanelPosition(UDim2.new(StartPosition.X.Scale, StartPosition.X.Offset + Delta.X, StartPosition.Y.Scale, StartPosition.Y.Offset + Delta.Y), PanelSize)
        ...
        InputService.InputEnded:Connect(function(Input)
            ...
            if Library.PerformanceDrag then
                DragOutline.Visible = false
                Outer.Position = DragOutline.Position
                MarkManuallyPositioned()
            end
        end)
    end

    local function CreateFilterBox(Parent, Position, Size, Placeholder, OnChanged)
        local Outer = Library:Create("Frame", {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Position = Position;
    ...
    local RelationsPath = "starhook_relations.json"

    local function LoadRelations()
        if not isfile(RelationsPath) then
            return
        end

        local Success, Data = pcall(function()
            return HttpService:JSONDecode(readfile(RelationsPath))
        end)
        ...
        for _, UserId in next, Data.Friends or {} do
            Library.Friends[UserId] = true
        end

        for _, UserId in next, Data.Priorities or {} do
            Library.Priorities[UserId] = true
        end
    end

    local function SaveRelations()
        local Friends, Priorities = {}, {}

        for UserId in next, Library.Friends do
            table.insert(Friends, UserId)
        end

        for UserId in next, Library.Priorities do
            table.insert(Priorities, UserId)
        end

        pcall(writefile, RelationsPath, HttpService:JSONEncode({
            Friends = Friends;
    ...
    function Library:GetPlayerInfoText(Player)
        return "Name: " .. Player.Name
    end

    function Library:CreatePlayerList()
        local PanelWidth, PanelHeight = 500 * DPIScale, 290 * DPIScale
        local ListWidth = PanelWidth * 0.65
        local IconSize = 110 * DPIScale

        local Outer, Body = CreatePanel("Player List", PanelWidth, PanelHeight)
        local SetAttached, MarkManuallyPositioned, ResetPosition = AttachToWindow(Outer, PanelWidth, "Right")

        MakePanelDraggable(Outer, 25, MarkManuallyPositioned)

        Library.ResetPlayerListPosition = ResetPosition
        ...
        local function ApplyFilter()
            for _, Row in next, Rows do
                Row.Button.Visible = (Filter == "") or string.find(string.lower(Row.Player.Name), Filter, 1, true) ~= nil
            end
        end

        CreateFilterBox(Body, UDim2.new(0, 6, 0, 4), UDim2.new(1, -12, 0, 20), "Filter by name...", function(Text)
        ...
        local Scrolling = Library:Create("ScrollingFrame", {
            ...
            Size = UDim2.new(0, ListWidth, 1, -56);
            CanvasSize = UDim2.new(0, 0, 0, 0);
        ...
        local IconOuter = Library:Create("Frame", {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Position = UDim2.new(1, -IconSize - 6, 0, 28);
            Size = UDim2.new(0, IconSize, 0, IconSize);
            ZIndex = 2;
            Parent = Body;
        })

        local IconInner = Library:Create("Frame", {
            BackgroundColor3 = Library.MainColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
            Size = UDim2.new(1, 0, 1, 0);
        ...
        Library:AddToRegistry(IconOuter, {
            BorderColor3 = "Black";
        })

        Library:AddToRegistry(IconInner, {
            BackgroundColor3 = "MainColor";
            BorderColor3 = "OutlineColor";
        })
        ...
        local InfoLabel = Library:CreateLabel({
            Position = UDim2.new(1, -IconSize - 6, 0, 28 + IconSize + 6);
            Size = UDim2.new(0, IconSize, 0, 80);
        ...
        local FriendOuter = CreatePanelButton(Body, "Friend", UDim2.new(0, 6, 1, -26), UDim2.new(0.5, -9, 0, 20))
        local PriorityOuter = CreatePanelButton(Body, "Priority", UDim2.new(0.5, 3, 1, -26), UDim2.new(0.5, -9, 0, 20))

        local Selected = nil

        local function GetStatusColor(Player)
            if Library.Priorities[Player.UserId] then
                return Color3.fromRGB(255, 210, 0)
            elseif Library.Friends[Player.UserId] then
        ...
        local function GetStatusText(Player)
            if Player == LocalPlayer then
                return "Local Player"
            elseif Library.Priorities[Player.UserId] then
                return "Priority"
            elseif Library.Friends[Player.UserId] then
                return "Friend"
            end
        ...
        local function GetRowBorder(Player)
            return (Selected == Player) and Library.AccentColor or Library.OutlineColor
        end
        ...
        local function UpdateRow(Player)
            ...
            Row.Status.Text = GetStatusText(Player)
            Row.Status.TextColor3 = GetStatusColor(Player)
            Row.Team.Text = Player.Team and Player.Team.Name or "None"
            Row.Team.TextColor3 = Player.Team and Player.TeamColor.Color or Color3.new(1, 1, 1)
            Row.Name.TextColor3 = (Selected == Player) and Library.AccentColor or Color3.new(1, 1, 1)
            Row.Button.BorderColor3 = GetRowBorder(Player)
        end
        ...
        local function SelectPlayer(Player)
            ...
            InfoLabel.Text = Library:GetPlayerInfoText(Player)
        end
        ...
        local function AddRow(Player)
            ...
            local Name = Library:CreateLabel({
            ...
            local Team = Library:CreateLabel({
            ...
            local Status = Library:CreateLabel({
                ...
                Size = UDim2.new(0.3, -4, 1, 0);
                Text = GetStatusText(Player);
                TextColor3 = GetStatusColor(Player);
                TextSize = 12;
            ...
            Button.MouseLeave:Connect(function()
                Button.BorderColor3 = GetRowBorder(Player)
            end)
        ...
        FriendOuter.MouseButton1Click:Connect(function()
            if not Selected or Selected == LocalPlayer then
                Library:Notify("Select a player first.", 3)
                return
            end

            Library.Priorities[Selected.UserId] = nil

            if Library.Friends[Selected.UserId] then
                Library.Friends[Selected.UserId] = nil
                Library:Notify(Selected.Name .. " is no longer a friend.", 3)
            else
                Library.Friends[Selected.UserId] = true
                Library:Notify(Selected.Name .. " marked as a friend.", 3)
            end
        ...
        PriorityOuter.MouseButton1Click:Connect(function()
            if not Selected or Selected == LocalPlayer then
                Library:Notify("Select a player first.", 3)
                return
            end
            ...
            if Library.Priorities[Selected.UserId] then
                Library.Priorities[Selected.UserId] = nil
                Library:Notify(Selected.Name .. " is no longer prioritized.", 3)
            else
                Library.Priorities[Selected.UserId] = true
                Library:Notify(Selected.Name .. " marked as priority.", 3)
            end
        ...
        Library.PlayerListPanel = Outer

        function Library:SetPlayerListVisibility(Bool)
            SetAttached(Bool)
        end
    end

    function Library:CreateServerList()
        local TeleportService = cloneref(game:GetService("TeleportService"))

        local PanelWidth, PanelHeight = 500 * DPIScale, 310 * DPIScale

        local Outer, Body = CreatePanel("Server List", PanelWidth, PanelHeight)
        local SetAttached, MarkManuallyPositioned, ResetPosition = AttachToWindow(Outer, PanelWidth, "Left")

        MakePanelDraggable(Outer, 25, MarkManuallyPositioned)

        Library.ResetServerListPosition = ResetPosition

        local Rows = {}
        local Filter = ""

        local function ApplyFilter()
            for Id, Row in next, Rows do
                Row.Button.Visible = (Filter == "") or string.find(string.lower(Id), Filter, 1, true) ~= nil
            end
        end

        CreateFilterBox(Body, UDim2.new(0, 6, 0, 4), UDim2.new(1, -12, 0, 20), "Filter by server id...", function(Text)
            Filter = string.lower(Text)
            ApplyFilter()
        end)
        ...
        local ConnectOuter, ConnectLabel = CreatePanelButton(Body, "Connect", UDim2.new(0, 6, 1, -26), UDim2.new(0.5, -9, 0, 20))
        local SortOuter, SortLabel = CreatePanelButton(Body, "Sort: Players", UDim2.new(0.5, 3, 1, -26), UDim2.new(0.35, -6, 0, 20))
        local RefreshOuter = CreatePanelButton(Body, "Refresh", UDim2.new(0.85, 3, 1, -26), UDim2.new(0.15, -9, 0, 20))

        local Selected = nil
        local Loading = false
        local SortByPing = false
        ...
        local function SelectServer(Id, Row)
            if Selected and Rows[Selected] then
                Rows[Selected].Name.TextColor3 = Color3.new(1, 1, 1)
                Rows[Selected].Button.BorderColor3 = Library.OutlineColor
            end
            ...
            Selected = Id
            Row.Name.TextColor3 = Library.AccentColor
            Row.Button.BorderColor3 = Library.AccentColor
        end
        ...
        local function AddRow(Server)
            ...
            Library:CreateLabel({
                ...
                Text = math.floor(Server.ping or 0) .. "ms";
                TextSize = 12;
                TextXAlignment = Enum.TextXAlignment.Right;
                ZIndex = 4;
                Parent = Button;
            ...
            local Row = { Button = Button; Name = Name; }
            Rows[Server.id] = Row
            ...
            Button.MouseLeave:Connect(function()
                Button.BorderColor3 = (Selected == Server.id) and Library.AccentColor or Library.OutlineColor
            end)

            Button.MouseButton1Click:Connect(function()
                SelectServer(Server.id, Row)
            end)

            ApplyFilter()
        end

        local function CompareServers(A, B)
            if SortByPing then
        ...
        local function RefreshServers()
            ...
            task.spawn(function()
                local Success, Result = pcall(function()
                    local Response = game:HttpGet(string.format("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=Asc&limit=100", game.PlaceId))
                    return HttpService:JSONDecode(Response).data
                end)

                if Success and Result then
                    table.sort(Result, CompareServers)

                    for _, Server in next, Result do
        ...
        RefreshOuter.MouseButton1Click:Connect(RefreshServers)

        SortOuter.MouseButton1Click:Connect(function()
            if SortAscending then
                SortAscending = false
            elseif SortByPing then
                SortByPing = false
                SortAscending = true
            else
                SortByPing = true
            ...
            SortLabel.Text = "Sort: " .. (SortByPing and "Ping" or "Players") .. (SortAscending and " asc" or " desc")
            RefreshServers()
        end)
        ...
        ConnectOuter.MouseButton1Click:Connect(function()
            if not Selected then
                Library:Notify("Select a server first.", 3)
                return
            end
            ...
            if Armed then
                ...
                TeleportService:TeleportToPlaceInstance(game.PlaceId, Selected)
                return
            end
            ...
            task.spawn(function()
                for Step = 3, 1, -1 do
                    ...
                    ConnectLabel.Text = "Confirm? " .. Step
                    task.wait(1)
                end
        ...
        Library.ServerListPanel = Outer

        function Library:SetServerListVisibility(Bool)
            SetAttached(Bool)
        end
    end
end
