...
do
	local CompatInput = cloneref(game:GetService("UserInputService"));
	local CompatKeybinds = {};

	local function CompatKeyName(Key)
		if Key == nil then
			return "None";
	...
	local function CompatResolveKey(Default)
		if not Default or Default == "None" then
			return nil;
		elseif Default == "MouseLeft" or Default == "MouseRight" then
			return Default;
		end;

		local Success, KeyCode = pcall(function() return Enum.KeyCode[Default]; end);
	...
	local function CompatKeyDown(Key)
		if not Key then
			return false;
		elseif Key == "MouseLeft" then
			return CompatInput:IsMouseButtonPressed(Enum.UserInputType.MouseButton1);
		elseif Key == "MouseRight" then
			return CompatInput:IsMouseButtonPressed(Enum.UserInputType.MouseButton2);
		else
			return CompatInput:IsKeyDown(Key);
		end;
	end;

	CompatInput.InputBegan:Connect(function(Input, GameProcessed)
		...
		for _, KeyProxy in next, CompatKeybinds do
			if KeyProxy.__Key == PressedKey and KeyProxy.Mode == "Toggle" then
				KeyProxy.Toggled = not KeyProxy.Toggled;
			end;
		end;
	end);

	local function CompatWrapValue(Response, Callbacks)
		local Proxy = {};
	...
	local function CompatWrapColorPicker(Response, Callbacks)
		local Proxy = {};
	...
	local function CompatWrapKeyPicker(Response, Info, Idx)
		local Proxy = {
			Mode = Info.Mode or "Toggle",
			Toggled = false,
			__Key = CompatResolveKey(Info.Default),
		};

		function Proxy:GetState()
			if Proxy.Mode == "Always" then
				return true;
			elseif Proxy.Mode == "Hold" then
				return CompatKeyDown(Proxy.__Key);
			else
		...
		setmetatable(Proxy, {
			__index = function(_, Key)
				if Key == "Value" then
					return CompatKeyName(Proxy.__Key);
				end;
		...
		CompatKeybinds[Idx] = Proxy;

		return Proxy;
	end;

	local CompatWrapSection;

	local function CompatWrapToggle(Response, Callbacks)
		local OptionSection = CompatWrapSection(Response.Option);
		local Proxy = {};
	...
	CompatWrapSection = function(RawElements)
		local Section = {};

		function Section:AddToggle(Idx, Info)
			local Callbacks = {};

			local Response = RawElements:AddToggle({
				Name = Info.Text,
				Default = Info.Default or false,
				Flag = Idx,
				Option = true,
				Callback = function(Value)
					for _, Fn in next, Callbacks do
						Fn(Value);
					end;
				end,
			});

			local Proxy = CompatWrapToggle(Response, Callbacks);
			Toggles[Idx] = Proxy;
		...
		function Section:AddSlider(Idx, Info)
			...
			local Response = RawElements:AddSlider({
				Name = Info.Text,
			...
			local Proxy = CompatWrapValue(Response, Callbacks);
			Options[Idx] = Proxy;
		...
		function Section:AddDropdown(Idx, Info)
			...
			if not Info.Multi and type(Default) == "number" then
				Default = Values[Default];
			end;

			local Response = RawElements:AddDropdown({
				Name = Info.Text,
				Default = Default,
				Values = Values,
				Multi = Info.Multi,
		...
		function Section:AddColorPicker(Idx, Info)
			...
			local Response = RawElements:AddColorPicker({
				Name = Info.Text,
				Default = Info.Default,
			...
			local Proxy = CompatWrapColorPicker(Response, Callbacks);
			Options[Idx] = Proxy;
		...
		function Section:AddKeyPicker(Idx, Info)
			...
			local Response = RawElements:AddKeybind({
				Name = Info.Text,
				Default = CompatResolveKey(Info.Default),
				Flag = Idx,
				Callback = function(KeyName)
					if Proxy then
						Proxy.__Key = CompatResolveKey(CompatKeyName(KeyName));
					end;
				end,
			});

			Proxy = CompatWrapKeyPicker(Response, Info, Idx);
			Options[Idx] = Proxy;

			return Section;
		end;

		function Section:AddButton(Text, Func)
			RawElements:AddButton({ Name = Text, Callback = Func });
		end;

		function Section:AddDivider() end;

		return Section;
	end;

	function Fatality:CreateWindow(Config)
		Config = Config or {};

		local WindowConfig = {
			Name = Config.Title or Config.Name or "Window",
			Scale = Config.Scale or UDim2.new(0, 700, 0, 520),
			Expire = Config.Expire or "never",
		};

		local Fatal = Fatality.new(WindowConfig);
		...
		local Window = {
			...
			AccentColor = Color3.fromRGB(106, 136, 213),
			KeybindFrame = { Visible = false },
			Assets = {},
		};

		function Window:AddTab(Name)
			local Menu = Fatal:AddMenu({ Name = Name, Icon = "circle", AutoFill = false });

			local Tab = {};

			function Tab:AddLeftGroupbox(SectionName)
				return CompatWrapSection(Menu:AddSection({ Name = SectionName, Position = "left" }));
			end;

			function Tab:AddCenterGroupbox(SectionName)
				return CompatWrapSection(Menu:AddSection({ Name = SectionName, Position = "center" }));
			end;

			function Tab:AddRightGroupbox(SectionName)
				return CompatWrapSection(Menu:AddSection({ Name = SectionName, Position = "right" }));
			end;
		...
		function Window:Toggle()
			...
			Fatal:SetVisible(WindowVisible);
		end;

		function Window:Notify(Text, Time)
			local Notifier = Fatality:CreateNotifier();
			Notifier:Notify({ Title = WindowConfig.Name, Content = Text, Duration = Time or 3, Icon = "info" });
		end;

		function Window:SetWatermark(Text) end;
		function Window:SetWatermarkVisibility(Bool) end;
		...
		function Window:Unload()
			...
			pcall(function() Fatal:SetVisible(false); end);
		end;
